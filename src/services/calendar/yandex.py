...
class YandexCalendarClient(BaseCalendarClient):
    ...
    def __init__(self, username: str, password: str):
        super().__init__(
            url="https://caldav.yandex.ru", username=username, password=password
        )
    ...
    def create_event(self, event: ParsedEvent) -> str:
        ...
        self.logger.info(f"Using Yandex calendar: {calendar.name} ({calendar.url})")
...
END:VCALENDAR"""
        ...
        self.logger.debug(f"iCal content for Yandex:\n{ical}")
        calendar.save_event(ical)
        self.logger.info(
            f"Created Yandex event: {event.title} on {event.start_date} at {event.start_time} {settings.timezone}"
        )
        return event.title
