...
class ICloudCalendarClient(BaseCalendarClient):
    """iCloud Calendar CalDAV client."""
    ...
    def create_event(self, event: ParsedEvent) -> str:
        ...
        if not calendars:
            raise ValueError("No calendars found in iCloud account")

        calendar = calendars[0]
        self.logger.info(f"Using iCloud calendar: {calendar.name} ({calendar.url})")
        ...
        if event.end_date and event.end_time:
            end_dt = tz.localize(
                datetime.strptime(
                    f"{event.end_date} {event.end_time}", "%Y-%m-%d %H:%M"
                )
            )
        else:
...
END:VCALENDAR"""
        ...
        self.logger.debug(f"iCal content for iCloud:\n{ical}")
        calendar.save_event(ical)
        self.logger.info(
            f"Created iCloud event: {event.title} on {event.start_date} at {event.start_time} {settings.timezone}"
        )
        return event.title
