...
class ICloudCalendarClient(BaseCalendarClient):
    ...
    def create_event(self, event: ParsedEvent) -> str:
        ...
        logger.info(f"Using iCloud calendar: {calendar.name} ({calendar.url})")
...
END:VCALENDAR"""
        ...
        logger.debug(f"iCal content for iCloud:\n{ical}")
        calendar.save_event(ical)
        logger.info(f"Created iCloud event: {event.title} on {event.start_date} at {event.start_time} {settings.timezone}")
        return event.title