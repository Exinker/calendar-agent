...
from time_utils import get_current_datetime_str, validate_date_range
...
async def parse_events(message: str) -> ParsedEventsList:
    async with httpx.AsyncClient() as client:
        response = await client.post(
            "https://openrouter.ai/api/v1/chat/completions",
            headers={
                "Authorization": f"Bearer {settings.openrouter_api_key}",
                "HTTP-Referer": "https://calemdar-agent.local",
                "X-Title": "Calemdar Agent",
